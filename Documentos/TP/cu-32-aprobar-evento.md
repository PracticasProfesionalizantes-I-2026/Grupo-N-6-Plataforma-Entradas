...
| **Stakeholders e intereses** | Admin → aprobar evento válido; Creador → que su evento se publique; Usuario visitante → ver evento aprobado; Sistema → habilitar venta de entradas |
| **Disparador (Trigger)** | El administrador selecciona "Aprobar" en un evento pendiente de revisión |
...
### 1. BREVE DESCRIPCIÓN
Permite al administrador aprobar un evento con estado "Pendiente de aprobación", cambiando su estado a "Aprobado" y habilitando su visibilidad pública y venta de entradas.
...
1. El Actor envía una petición al endpoint `PUT /api/admin/eventos/{id}/aprobar` con header Authorization.
2. La **Capa de Presentación** valida el JWT, rol "Admin", y ID (GUID).
...
6. El Sistema notifica al creador del evento (email/in-app): "Tu evento fue aprobado".
...
* **5a. Error interno (HTTP 500 Internal Server Error):**
  1. Si en el Paso 5 falla la persistencia o notificación.
...
3. Creador notificado de la aprobación.
4. Registro de auditoría: adminId, fecha, acción "Aprobar".
...
- **Validación (Presentación, → 400/401):** JWT válido, rol Admin en claims, formato ID (GUID).
...
| --- | --- | --- | --- |
| Flujo principal | `200 OK` | `AprobarEvento_WithValidEvento_UpdatesStateAndNotifies` | `PutAprobarEvento_WithValidId_Returns200OK` |
| 1a. Token inválido | `401 Unauthorized` | — | `PutAprobarEvento_WithInvalidToken_Returns401Unauthorized` |
| 1b. Sin rol Admin | `403 Forbidden` | — | `PutAprobarEvento_WithUserToken_Returns403Forbidden` |
