...
| **Stakeholders e intereses** | Super Admin → rechazar evento no válido; Creador → conocer motivo de rechazo; Sistema → mantener evento oculto |
| **Disparador (Trigger)** | El **Super Admin** selecciona "Rechazar" en un evento pendiente de revisión, ingresando motivo |
...
### 1. BREVE DESCRIPCIÓN
Permite al **Super Admin** rechazar un evento con estado "Pendiente de aprobación", cambiando su estado a "Rechazado" y notificando al creador con el motivo.
...
1. El **Super Admin** envía una petición al endpoint `PUT /api/admin/eventos/{id}/rechazar` con JSON (motivoRechazo) y header Authorization.
2. La **Capa de Presentación** valida el JWT, rol "SuperAdmin", ID (GUID), y que motivoRechazo no esté vacío (máx. 500 chars).
...
* **1b. Sin rol SuperAdmin (HTTP 403 Forbidden):**
...
3. 
4. Registro de auditoría: **superAdminId**, fecha, acción "Rechazar", motivo.
5. Según regla de negocio: **Una vez rechazado, un evento no puede volver a "Pendiente de aprobación" directamente; requiere una nueva creación o revisión administrativa completa**.
...
- **Validación (Presentación, → 400/401):** JWT válido, rol SuperAdmin, formato ID, motivoRechazo obligatorio y longitud.
...
| --- | --- | --- | --- |
| Flujo principal | `200 OK` | `RechazarEvento_WithValidMotivo_UpdatesState` | `PutRechazarEvento_WithValidData_Returns200OK` |
| 1a. Token inválido | `401 Unauthorized` | — | `PutRechazarEvento_WithInvalidToken_Returns401Unauthorized` |
| 1b. Sin rol SuperAdmin | `403 Forbidden` | — | `PutRechazarEvento_WithNonSuperAdminToken_Returns403Forbidden` |
